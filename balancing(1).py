...
def kickclient(ip,mac,token):
	...
	url = urlt+token
	data = '''{
		"params": ["ubus call hostapd.wlan0 del_client \\\" {'addr': '%s','reason': 5,'deauth': false,'ban_time': 3000}\\\""],
		"jsonrpc": "2.0",
		"id": 1,
		"method": "exec"
	}'''%mac
	...
	try:
		...
		for j in range(i):
			try:
				...
				#print("Mang loi, kiem tra day")
				print("Canh bao mang loi"+ip[j])
				ip.remove(ip[j])
				i-=1
		for j in range(i):
